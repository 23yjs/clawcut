...
- `run_mode`：运行模式，支持 `model`、`fallback`、`smoke`，默认 `model`。`model` 必须成功调用 Ark 三阶段。
- `allow_fallback`：显式允许模型失败时启发式兜底，默认关闭。
- `visual_input_mode`：视觉输入模式，支持 `video`、`clip`、`keyframes`，默认 `video`。
- `candidate_mode`：候选生成模式，支持 `scene`、`window`、`hybrid`、`llm_hybrid`，默认 `llm_hybrid`。
- `scoring_mode`：兼容旧参数，默认 `model`。
- `assembly_mode`：兼容旧参数，默认 `model`。
- `planner_video_max_seconds`：Planner 视频最长时长，默认 `180`。
- `planner_video_fps`：Planner 视频 fps，默认 `1`。
- `planner_video_width`：Planner 视频宽度，默认 `512`。
- `candidate_clip_width`：候选 clip 宽度，默认 `512`。
- `judge_batch_size`：Candidate Judge 每批候选 clip 数，默认 `2`。
...
1. 校验视频路径、目标时长、输出目录。
2. 使用 `ffprobe` 读取视频元信息，使用 `ffmpeg` 进行 scene detection，并生成压缩后的 `planner_video.mp4`。
3. Candidate Planner：Ark 接收 `planner_video.mp4` 的 video data URL，基于真实视频内容、用户指令、scene boundaries 和字幕规划语义候选切片，不输出 score。
4. 融合 `ffmpeg` scene/window 候选与大模型候选边界，生成 `candidates.json` 和 `candidate_evidence.json`。
5. 使用 `ffmpeg` 为每个候选切片生成 `candidate_clips/*.mp4`。
6. Candidate Judge：Ark 接收候选短视频 clip 的 video data URL，对每个候选切片独立评分，输出 `score`、`score_breakdown`、`covered_points`、`quality_issues` 和中文 `reason`。
7. Assembly Planner：Ark 基于候选评分、目标时长、平台风格和卖点覆盖生成 `assembly_plan.json`。
8. 校验拼接方案；`model` 模式下模型失败直接失败，只有显式 `fallback` / `allow_fallback` 才退回启发式兜底。
9. `ffmpeg` 只按 `assembly_plan.json` 执行裁剪、转码、拼接和比例转换，不参与语义评分。
10. 输出高光视频、时间线报告、片段 JSON、模型中间产物和评测兼容字段。
...
- `planner_video.mp4`：传给 Ark Candidate Planner 的压缩视频预览。
- `planner_video_info.json`：Planner 视频信息。
...
- `candidate_clips/`：传给 Ark Candidate Judge 的候选短视频 clip。
...
- `model` 模式下缺少 Ark 配置：直接失败并写入 `failure.json`。
- `model` 模式下 Candidate Planner / Judge / Assembly 任一阶段失败：直接失败，不允许 silent fallback。
- `fallback` 模式且显式 `allow_fallback` 时：模型失败才退回启发式，并在 `fallback.stages` / `fallback.reasons` 中记录原因。
- `smoke` 模式：不要求 Ark，用于本地调试。
